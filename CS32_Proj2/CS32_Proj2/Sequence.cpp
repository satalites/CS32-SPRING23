...
//    if(pos == 0) //if we are trying to remove head
//    {
//        iterator->forward = head->forward; //iterator holds the 2nd item's address
//        iterator->back = head->back; //iterator holds the last item's address
//        head->back->forward = iterator->forward; //the last item's forward is the 2nd item's address
//        head->forward->back = iterator->back; //the 2nd item's back becomes the last item's address
//        head = iterator->forward; //head points to 2nd item's address
//        delete iterator;
//        mSize--; //removed 1 item
...
{
    Node* iterator;
    int pos = 0; //tracks our position to give to erase function
    int tracker = 0; //tracks number of times erase is called
    for(iterator = head->forward; iterator != head; iterator = iterator->forward)
    {
...
int subsequence(const Sequence& seq1, const Sequence& seq2)
{
    ...
    {
        seqExists = true; //this may/may not change depending on execution of next for loop
...
void concatReverse(const Sequence& seq1, const Sequence& seq2, Sequence& result)
{
    ...
    {
        seq1.get(i,toAdd);
        result.insert(j,toAdd);
    }
    ItemType toAdd2;
    for(int i = seq2.size()-1, j = result.size(); i >= 0; i--, j++)
    {
        seq2.get(i,toAdd2);
        result.insert(j,toAdd2);
    }
    return;
}
