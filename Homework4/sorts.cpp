...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
    ...
    {
        int loc = i;
        while(comp(s[loc], s[loc-1]) && loc > 0)
        {
            swap(s.at(loc-1),s.at(loc));
            loc--;
        }
    }
}
...
//    vector<Store*>::iterator beginning = storePtrs.begin();
//    vector<Store*>::iterator ending = storePtrs.end();
//    sort(beginning, ending, comp);
