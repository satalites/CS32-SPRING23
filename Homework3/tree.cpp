...
int countIncludes(const string a1[], int n1, const string a2[], int n2)
{
    ...
    if(a1[0] != a2[0])
    {
        return countIncludes(a1+1, n1-1, a2, n2); //move onto next value in a1 if they dont match up
    }
    else
    {
        return countIncludes(a1+1, n1-1, a2, n2) + countIncludes(a1+1, n1-1, a2+1, n2-1); //if they match, go up one value in both arrays, and then add that to another recursive call that sees
    }
}
...
void order(string a[], int n)
{
    ...
    int firstNotLess = 0;
    int separator = n/2;
    separate(a, n, a[separator], firstNotLess, firstGreater);
